...
describe('Cinema', function () {
  ...
  it('should be able to find a film by genre', function() {
    const actual = cinema.findFilmByProperty('genre', 'drama');
  ...
  it('should be able to check whether all films are over a particular length', function(){
    const actual = cinema.findFilmByLength(90);
    assert.deepStrictEqual(actual, true);
  });
  ...
  it('should be able to find a film by year', function() {
    const actual = cinema.findFilmByProperty('year', 2016);
    assert.deepStrictEqual(actual, [moonlight]);
  });
